...
from I2_metrics_scripts import i2_metrics
...
#                                       comment="#", header=0, sep='\t',index_col=0)
        ...
        # metrics calculated from input files
        window = 15
        sensitivity, fdr = i2_metrics.calculate_metrics(input_participant,metrics_data,window)
        ...
        data_id_1 = community + ":" + challenge + "_sensitivity_" + participant + "_A"
        std_error= 0
        assessment_sensitivity = JSON_templates.write_assessment_dataset(data_id_1, community, challenge, participant, "Sensitivity", sensitivity, std_error)

        data_id_2 = community + ":" + challenge + "_FDR_" + participant + "_A"
        std_error= 0
        assessment_FDR = JSON_templates.write_assessment_dataset(data_id_2, community, challenge, participant, "FDR", fdr, std_error)

        # push the two assessment datasets to the main dataset array
        ALL_ASSESSMENTS.extend([assessment_sensitivity, assessment_FDR])
